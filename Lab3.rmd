...
We find high statistical significance for the arrest probability coefficient; very high significance for the population density coefficient.  Each unit increase in population density seems to bring 0.007 crimes with him or her.  Each percent increase in arrest probability takes away .0005 crimes.  We note that these numbers are a little difficult to comprehend practically.  At this point, let us examine the diagnostic plots for our model.

```{r model1_plots, fig.height=9}
autoplot(m1, which = 1:6)
```

We see from the residuals vs fitted values plot that there is some evidence of non-zero conditional error mean.  In addition the Q-Q plot shows some evidence of positive residual skew.  At this point, we consider transforming the output variable into $\log(crmrte)$.  The interpretation of the new output variable is the proportional change in crime rate with respect to differential changes in input variables.  This interpretation makes logical sense (e.g. a 10% increase in crime rate is easy to comprehend).
...
```
We find virtually no difference with the logit transformed probability.  We will forego it in favor of ease of interprebability.  Now we return to the diagnostic plots of model 2.

```{r model2_plot, fig.height=9}
autoplot(m2, which = 1:6)
```

We now see a much better residuals vs fitted values plot (much closer to zero conditional mean).  The skew in our Q-Q plot also seems to have improved.  The data actually looks fairly homoskedastic from the scale-location plot, albeit model 1 had decent homoskedasticity as well.  We see that row 49 has a fairly high leverage, but its Cook's distance is not high enough for us to warrant investigating it further.
...
```{r model4}
m4 = lm(log(t2$crmrte) ~ t2$prbarr + t2$prbconv + t2$prbpris + t2$avgsen + t2$density + 
            t2$taxpc + t2$pctmin80 + t2$mix + t2$pctymle + t2$wage)
coeftest(m4, vcov. = vcovHC)
```
The density coefficient is still statistically significant, despite adding in multiple correlated variables.  We did gain any additional high significance predictors.
