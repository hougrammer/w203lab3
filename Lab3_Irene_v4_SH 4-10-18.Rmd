...
$$ crmrate = \beta_0 + (\beta_1\cdot prbarr) + (\beta_2\cdot prconv)+ (\beta_3\cdot density)+ (\beta_4\cdot taxpc)+ 
(\beta_5\cdot pctmin80)+ (\beta_6\cdot pctymle) + (\beta_k\cdot x_k) \  + u$$


$$ x_k = \delta_0 + (\delta_1\cdot prbarr) + (\delta_2\cdot prconv)+ (\delta_3\cdot density)+ (\delta_4\cdot taxpc) + (\delta_5\cdot pctmin80) + (\delta_6\cdot pctymle) + v$$
...
Morals represents a person's conscious and upbringing, as well as their ability to empathize with others. A person that has greater morals, is likely to commit less crime than a person with little to no morals. Therefore, beta_k is less than 0 (higher morals equates to lower crime rate). A person with higher morals is also less likely to be arrested and convicted, therefore delta_1 and delta_1 are less than zero. Morals has no impact on taxpc, pctmin80, or pctymle, therefore delta_4, delta_5, and delta_6 are zero. 
