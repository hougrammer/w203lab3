...
m2_wage = lm(t$crmrte ~ t$wcon + t$wtuc + t$wtrd + t$wfir + t$wser + 
               t$wmfg + t$wfed + t$wsta + t$wloc)
...
m3 = lm(t$crmrte ~ t$prbarr + t$prbconv + t$prbpris + t$avgsen + t$density + t$taxpc + 
          t$pctmin80 + t$mix + t$pctymle + t$wage)
...
$$ crmrate = \beta_0 + \beta_1\cdot prbarr + \beta_2\cdot prconv+ \beta_3\cdot density+ \beta_4\cdot taxpc+ \beta_5\cdot pctmin80+ \beta_6\cdot pctymle + error$$
...
The OLS coeffecient will be more positive, therefore gaining statistical significance. The OLS coeffecient was underestimated.
...
The study concluded that the population density variable was the best predictor for crime rate. Our model 2 was able to include a few other variables that increased the accuracy without introducing substantial bias.  However, it is unlikely that any political platform could make a significant effect on reducing crime rate, since population density (the most correlated variable) is a difficult task to address.   

Of the three probabilities of punishment (arrest, conviction, sentence), arrest has the highest impact on crime rate.  Police per capita and crime rate were positively correlated, concluding that increasing police per capita did not reduce crime rate.  It may be more prudent to dedicate more resources for the existing police force so that more arrests can be made without necessarily increasing police presence. An example of this would be improved training, improved supplies, and better relationships between the police force and the population. 
